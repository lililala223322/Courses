...
double Speed(double temp, double pressure, int elevation, int runway, int weight, int flaps, int wet) {
    // 检查输入是否在有效范围内
    ...
    // 计算温度档和气压档
    int Temp = floor(temp / 10);
    int Pressure = ceil(pressure);
 
    // 检查操纵参考表是否存在
    if (Temp < 0 || Temp > 7 || Pressure < 0 || Pressure > 9) {
        return -1;
    }
    ...
    char reference = referenceTable[Temp][Pressure];
...
int main()
{
    ...
    scanf("%lf %lf",&temp,&pressure);
    scanf("%d %d %d %d %d",&elevation,&runway,&weight,&flaps,&wet);
  
    int result=Speed(temp,pressure,elevation,runway,weight,flaps,wet);
    if (result == -1) {
